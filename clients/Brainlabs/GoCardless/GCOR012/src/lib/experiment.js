...
const controlClassPrefix = 'GCOR008__howitworks--';

const startReelBtnData = {
  text: 'Watch an example',
  link: '',
...
const init = () => {
  ...
  if (!window.gco008__Pagetypes[window.location.pathname]) {
    return;
  }
  ...
  //render stories reel
  const controlGetStartedBtn = document.querySelector(`.${controlClassPrefix}cta`);
  ...
  pollerLite([() => typeof window.Swiper === 'function'], () => {
    ...
    controlGetStartedBtn.insertAdjacentHTML('beforebegin', reelBtn(ID, startReelBtnData));
...
export default () => {
  ...
  pollerLite(['#___gatsby', `.${controlClassPrefix}heroImage>.image-wrapper`], () => {
    ...
    document.body.addEventListener('click', ({ target }) => {
      ...
      const articleType = window.gco008__Pagetypes[window.location.pathname];

      if (!articleType) {
        return;
      }
      const overlay = document.querySelector(`.${ID}__overlay`);
      const module3 = document.querySelector(`.${controlClassPrefix}module3`);
      const swiperPagination = document.querySelector('.swiper-pagination');

      const initiateReel = () => {
        overlay?.classList.remove(`${ID}__hide`);
        initSwiper('.swiper', fireEvent, shared);
        module3.classList.add(`${ID}__margin-0`);
